...
class Twenty_Twenty_One_Dark_Mode {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'from21_html_classes', array( $this, 'html_classes' ) );
	...
	public function editor_custom_color_variables() {
		...
		wp_enqueue_script(
			'from21-dark-mode-support-toggle',
		...
		wp_enqueue_script(
			'from21-editor-dark-mode-support',
			get_template_directory_uri() . '/assets/js/editor-dark-mode-support.js',
			array( 'from21-dark-mode-support-toggle' ),
			'1.0.0',
			array( 'in_footer' => true )
		);
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			'from21-customize-controls',
			get_template_directory_uri() . '/assets/js/customize.js',
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'from21-customize-helpers' ),
			'1.0.0',
			array( 'in_footer' => true )
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'from21' );
		}
		...
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom corner of the page.', 'from21' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'from21' ),
				'priority'        => 110,
	...
	public function admin_body_classes( $classes ) {
		...
		if ( $current_screen->is_block_editor() ) {
			...
			if ( $should_respect_color_scheme && Twenty_Twenty_One_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
				$classes .= ' from21-supports-dark-theme';
			}
		}
	...
	public function the_html( $attrs = array() ) {
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'from21' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'from21' ); ?>';
			}
