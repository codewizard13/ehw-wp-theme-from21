...
function twenty_twenty_one_continue_reading_text() {
	$continue_reading = sprintf(
		...
		esc_html__( 'Continue reading %s', 'from21' ),
...
if ( ! function_exists( 'twenty_twenty_one_post_title' ) ) {
	...
	function twenty_twenty_one_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'from21' ) : $title;
	}
}
 ...
 *
 * @since From21 1.0
 * @since From21 1.4 Corrected parameter name for `$output`,
...
function twenty_twenty_one_password_form( $output, $post = 0 ) {
	...
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'from21' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'from21' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" spellcheck="false" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'from21' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'from21' ) . '" /></form>
	';
	return $output;
}
